...
void imprimeAsFuncoesDasTeclas() {
    ...
    cout << "S      | Desenha o componente Suzanne3D" << endl;
...
void* janelaOpenGL(void* arg) {
    ...
    GLFWwindow* window = glfwCreateWindow(1024, 768, "M4 - Adicionando Iluminação", NULL, NULL); // Lunaris
    if (window == NULL) {
    ...
    Planeta3D planeta = Planeta3D(window, projection);
    Suzanne3D suzanne = Suzanne3D(window, projection);

    // Indica se os componentes devem ou não ser desenhados:
    bool desenhaGrade2D = false;
    bool desenhaPlaneta3D = false;
    bool desenhaSuzanne3D = true;
    ...
    while(!glfwWindowShouldClose(window)) { // Loop que mantém o desenho da janela principal --> Se glfwWindowShouldClose(window) retorna 'true' o loop pára e o programa encerra
       ...
       // glClear(GL_COLOR_BUFFER_BIT); // Aplica as cores acima ao buffer de cor -- poderia aplicar as cores acima a outros buffers (GL_DEPTH_BUFFER_BIT OU GL_STENCIL_BUFFER_BIT)
        ...
        const int desenhaSuzanne3DTecla = glfwGetKey(window, GLFW_KEY_S);
        if (desenhaSuzanne3DTecla == GLFW_PRESS) {
            desenhaSuzanne3D = true;
        }


        if (desenhaPlaneta3D) planeta.desenhar();
        if (desenhaSuzanne3D) suzanne.desenhar();

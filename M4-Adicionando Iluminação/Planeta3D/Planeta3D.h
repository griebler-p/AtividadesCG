        ...
        void popularVertices(){
            ...
            for (int c=0; c<=cir_f; c++) { // Itera pelos círculos
                for (int p=0; p<=pos_f; p++) { // Itera pelas posições // Não deve pular de 2 em 2 pq os quadrados devem se encostar
                    ...
                    //vertices[vert+35] =  0.0; // Cor B do V3 // Original*/

                    /*// Inversão de posição do V2 com V3 para tentar corrigir o erro do triângulo
                    vertices[vert+30] = circulos[c+1][p+1][0]/(r_f*1.1); // Coord. X do V2 -- / por 110% do raio para ficar dentro dos limites -1.0 e +1.0
                    vertices[vert+31] = circulos[c+1][p+1][1]/(r_f*1.1); // Coord. Y do V2
                    vertices[vert+32] = circulos[c+1][p+1][2]/(r_f*1.1); // Coord. Z do V2
                    //vertices[vert+27] =  1.0; // Cor R do V1
                    //vertices[vert+28] =  1.0; // Cor G do V1
                    //vertices[vert+29] =  0.0; // Cor B do V1

                    vertices[vert+24] = circulos[c+1][p][0]/(r_f*1.1); // Coord. X do V3 -- / por 110% do raio para ficar dentro dos limites -1.0 e +1.0
                    vertices[vert+25] = circulos[c+1][p][1]/(r_f*1.1); // Coord. Y do V3
                    vertices[vert+26] = circulos[c+1][p][2]/(r_f*1.1); // Coord. Z do V3*/
